        ...
        <div className="flex flex-wrap">
          {seasonsData.map((seasonData) => (
            <button
              ...
              className={`flex-1 px-8 py-4 text-lg font-bold whitespace-nowrap transition-all duration-200 ${
      ...
      {currentSeasonData && (
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-700 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-purple-600 to-purple-500 px-6 py-4">
            <h2 className="text-2xl font-bold text-white">{currentSeasonData.season} Season</h2>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-900/50">
                <tr className="border-b border-slate-700">
                  ...
                  <th className="px-6 py-4 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">Points Against</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-700/50">
                {currentSeasonData.teams.map((team, index) => (
                  <tr key={team.userId} className="hover:bg-slate-700/30 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-green-400 font-bold">{formatNumber(team.pf)}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-red-400 font-bold">{formatNumber(team.pa)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
