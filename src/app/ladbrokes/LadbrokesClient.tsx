...
export default function LadbrokesClient() {
  ...
  useEffect(() => {
    // loadState waits for the API response before updating client state.
  ...
  const usesTiebreakers = state.week >= 3;
  const tiebreakersComplete = !usesTiebreakers || (
    highestScorerRosterId != null
      ...
      <div className="p-6 sm:p-8">
        ...
        {activeTab === "picks" && state.user && (
          <div>
            ...
            {usesTiebreakers && state.matchups.length > 0 && (
              <section className="tool-panel mt-6 p-5" aria-labelledby="ladbrokes-tiebreaker-title">
                <p className="eyebrow">Weekly tiebreaker</p>
                <h3 id="ladbrokes-tiebreaker-title" className="text-2xl font-bold uppercase text-white">Pick the scoring extremes</h3>
                <p className="mt-1 text-sm text-white/60">These decide the winner when game-pick totals are tied.</p>
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-2 text-sm font-bold text-white">
                    Highest scorer of the week
                    <select
                      value={highestScorerRosterId ?? ""}
                      onChange={(event) => setHighestScorerRosterId(Number(event.target.value))}
                      disabled={!!state.ownSubmission || state.picksLocked}
                      className="wheel-admin-field disabled:cursor-default disabled:opacity-70"
                    >
                      <option value="" disabled>Select a manager</option>
                      {state.owners.map((owner) => <option key={owner.rosterId} value={owner.rosterId}>{owner.displayName}</option>)}
                    </select>
                  </label>
                  <label className="grid gap-2 text-sm font-bold text-white">
                    Lowest scorer of the week
                    <select
                      value={lowestScorerRosterId ?? ""}
                      onChange={(event) => setLowestScorerRosterId(Number(event.target.value))}
                      disabled={!!state.ownSubmission || state.picksLocked}
                      className="wheel-admin-field disabled:cursor-default disabled:opacity-70"
                    >
                      <option value="" disabled>Select a manager</option>
                ...
                {highestScorerRosterId != null && highestScorerRosterId === lowestScorerRosterId && (
                  <p className="mt-3 text-sm text-orange-300">Choose a different manager for each tiebreaker.</p>
                )}
                {state.ownSubmission && state.revealedPicks && (
                  <div className="mt-5 border-t border-white/10 pt-4">
                    <p className="mb-3 text-xs font-bold uppercase text-white/50">Locked selections</p>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {state.revealedPicks.map((entry) => {
                        const highest = state.owners.find((owner) => owner.rosterId === entry.tiebreakers?.highestScorerRosterId)?.displayName;
                        const lowest = state.owners.find((owner) => owner.rosterId === entry.tiebreakers?.lowestScorerRosterId)?.displayName;
                        return <div key={entry.rosterId} className="border border-white/10 bg-black/20 p-3 text-sm"><strong className="block text-white">{entry.displayName}</strong><span className="block text-white/60">Highest: {highest ?? "Not selected"}</span><span className="block text-white/60">Lowest: {lowest ?? "Not selected"}</span></div>;
                      })}
                    </div>
                  </div>
                )}
        ...
        {activeTab === "results" && (
          <div>
            ...
            {state.history.length === 0 ? <div className="tool-panel p-8 text-center"><strong className="text-xl text-white">No completed weeks yet</strong><p className="mt-2 text-white/60">Week 1 results will appear here after Sleeper marks the matchups final.</p></div> : <>
              ...
              <div className="space-y-4">{state.history.map((week) => <section key={week.week} className="tool-panel p-5"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-white/10 pb-4"><div><p className="eyebrow">Week {week.week}</p><h3 className="text-2xl font-bold uppercase text-white">{week.winners.length === 0 ? "No entries" : week.winners.length === 1 ? `${week.winners[0].displayName} wins` : `${week.winners.map((winner) => winner.displayName).join(" & ")} tie`}</h3></div>{week.winners[0] && <strong className="text-lime-300">{week.winners[0].correct}/{week.standings[0]?.total ?? 0} correct{week.week >= 3 ? ` · ${week.winners[0].tiebreakCorrect}/2 tiebreakers` : ""}</strong>}</div>{week.standings.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{week.standings.map((entry, index) => <div key={entry.rosterId} className="flex justify-between border border-white/10 bg-black/20 px-3 py-2"><span className="text-white/80">{index + 1}. {entry.displayName}</span><strong className="text-white">{entry.correct}/{entry.total}{week.week >= 3 ? ` + ${entry.tiebreakCorrect}/2 TB` : ""}</strong></div>)}</div>}</section>)}</div>
            </>}
          </div>
        )}
